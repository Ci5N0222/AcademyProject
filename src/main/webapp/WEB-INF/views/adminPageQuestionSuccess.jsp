...
function fn_prev(page, range, rangeSize) {      
      ...
      var url = "${contextPath}/board/adminPageQuestionSuccess";      
      url = url + "?page=" + page;      
      url = url + "&range=" + range;            
      location.href = url;   
   }  
   ...
   <div class="container-fluid bg-primary py-5 bg-header" >
    ...
    <div class="container-fluid py-5" >
            <div class="section-title text-center position-relative pb-3 mb-5 mx-auto" style="max-width:500px;text-align:left;" >
                <h1 class="mb-0">문 의 처 리 완 료 내 역</h1>
            </div>
   </div>
<!-- title end-->
    ...
    <section class="content container-fluid">
      <div class="table-responsive">
        <table class="table" style="text-align:center">
         ...
         <c:forEach var="board" items="${adminPageQuestionSuccess}">
            <tr>
                <td>${board.bd_num}</td>
                <td><a href="detail?bd_num=${board.bd_num}"> ${board.bd_tl} </a></td>
                <td>${board.m_ename}</td>
                <td>${board.bd_dt}</td>
                <c:if test = "${board.q_end eq '2'}">
                <td><c:out value="처리대기"></c:out></td>
                </c:if>
                 <c:if test = "${board.q_end eq '3'}">
